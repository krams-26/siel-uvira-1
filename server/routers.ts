...
export const appRouter = router({
  ...
  schools: router({
    list: permissionGuard(["sous_proved", "secretariat", "chef_bureau", "inspecteur", "ecole"], "schools.view").input(z.object({ search: z.string().optional(), schoolType: z.string().optional(), level: z.string().optional(), status: z.string().optional(), territory: z.string().optional() }).optional()).query(async ({ input }) => { const db = await getDb(); if (!db) return []; const filters = []; if (input?.search) filters.push(or(like(schools.officialName, `%${input.search}%`), like(schools.code, `%${input.search}%`), like(schools.directorName, `%${input.search}%`))); if (input?.schoolType) filters.push(eq(schools.schoolType, input.schoolType)); if (input?.level) filters.push(eq(schools.level, input.level)); if (input?.status) filters.push(eq(schools.status, input.status)); if (input?.territory) filters.push(eq(schools.territory, input.territory)); return db.select().from(schools).where(filters.length ? and(...filters) : undefined).orderBy(desc(schools.updatedAt)); }),
  ...
  templates: router({
    generate: permissionGuard(["sous_proved", "secretariat", "ops"], "documents.create").input(z.object({ templateBody: z.string().min(10), variables: z.record(z.string(), z.string()) })).query(({ input }) => { let rendered = input.templateBody; for (const [key, value] of Object.entries(input.variables)) rendered = rendered.replaceAll(`{{${key}}}`, value); return { html: `<article style="font-family:Arial;white-space:pre-wrap">${rendered}</article>`, rendered }; }),
  ...
  statistics: router({
    summary: permissionGuard(["sous_proved", "secretariat", "inspecteur"], "statistics.view").input(z.object({ territory: z.string().optional(), level: z.string().optional(), status: z.string().optional() }).optional()).query(async ({ input }) => { const db = await getDb(); if (!db) return { schools: 0, students: 0, teachers: 0, dossiers: 0, reports: 0 }; const [allSchools, dossierRows, reportRows] = await Promise.all([db.select().from(schools), db.select().from(dossiers), db.select().from(reports)]); const schoolRows = allSchools.filter(s => (!input?.territory || s.territory === input.territory) && (!input?.level || s.level === input.level) && (!input?.status || s.status === input.status)); return { schools: schoolRows.length, students: schoolRows.reduce((sum, s) => sum + s.studentCount, 0), teachers: schoolRows.reduce((sum, s) => sum + s.teacherCount, 0), dossiers: dossierRows.length, reports: reportRows.length }; }),
    exportCsv: permissionGuard(["sous_proved", "secretariat", "inspecteur"], "statistics.export").input(z.object({ territory: z.string().optional(), level: z.string().optional(), status: z.string().optional() }).optional()).query(async ({ input }) => { const db = await getDb(); if (!db) return "indicateur;valeur\n"; const [allSchools, dossierRows, reportRows] = await Promise.all([db.select().from(schools), db.select().from(dossiers), db.select().from(reports)]); const schoolRows = allSchools.filter(s => (!input?.territory || s.territory === input.territory) && (!input?.level || s.level === input.level) && (!input?.status || s.status === input.status)); return ["indicateur;valeur", `ecoles;${schoolRows.length}`, `eleves;${schoolRows.reduce((sum, s) => sum + s.studentCount, 0)}`, `enseignants;${schoolRows.reduce((sum, s) => sum + s.teacherCount, 0)}`, `dossiers;${dossierRows.length}`, `rapports;${reportRows.length}`].join("\n"); }),
    exportFile: permissionGuard(["sous_proved", "secretariat", "inspecteur"], "statistics.export").input(z.object({ format: z.enum(["xlsx", "pdf"]), territory: z.string().optional(), level: z.string().optional(), status: z.string().optional() })).query(async ({ input }) => { const db = await getDb(); if (!db) return { format: input.format, base64: "", mimeType: "application/octet-stream" }; const [allSchools, dossierRows, reportRows] = await Promise.all([db.select().from(schools), db.select().from(dossiers), db.select().from(reports)]); const schoolRows = allSchools.filter(s => (!input.territory || s.territory === input.territory) && (!input.level || s.level === input.level) && (!input.status || s.status === input.status)); const rows = [{ indicateur: "Écoles", valeur: schoolRows.length }, { indicateur: "Élèves", valeur: schoolRows.reduce((sum, s) => sum + s.studentCount, 0) }, { indicateur: "Enseignants", valeur: schoolRows.reduce((sum, s) => sum + s.teacherCount, 0) }, { indicateur: "Dossiers", valeur: dossierRows.length }, { indicateur: "Rapports", valeur: reportRows.length }]; if (input.format === "xlsx") { const workbook = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(rows), "Statistiques"); return { format: input.format, base64: XLSX.write(workbook, { type: "base64", bookType: "xlsx" }), mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }; } const chunks: Buffer[] = []; const pdf = new PDFDocument(); pdf.on("data", (chunk: Buffer) => chunks.push(chunk)); const done = new Promise<void>(resolve => pdf.on("end", resolve)); pdf.fontSize(18).text("SIEL Uvira 1 — Statistiques", { align: "center" }).moveDown(); rows.forEach(row => pdf.fontSize(12).text(`${row.indicateur} : ${row.valeur}`)); pdf.end(); await done; return { format: input.format, base64: Buffer.concat(chunks).toString("base64"), mimeType: "application/pdf" }; }),
  }),
  administration: router({
    ...
    officesUpdate: permissionGuard(["admin", "sous_proved"], "audit.view").input(z.object({ id: z.number(), name: z.string().min(2).optional(), description: z.string().optional(), isActive: z.boolean().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const { id, ...changes } = input; await db.update(offices).set(changes).where(eq(offices.id, id)); await createAudit({ actorId: ctx.user.id, action: "OFFICE_UPDATED", entityType: "office", entityId: id, afterData: changes }); return { success: true }; }),
    officesCreate: permissionGuard(["admin", "sous_proved"], "audit.view").input(z.object({ code: z.string().min(2), name: z.string().min(2), description: z.string().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const result = await db.insert(offices).values(input); await createAudit({ actorId: ctx.user.id, action: "OFFICE_CREATED", entityType: "office", entityId: Number(result[0].insertId), afterData: input }); return { id: Number(result[0].insertId) }; }),
    users: permissionGuard(["admin", "sous_proved"], "audit.view").query(async () => { const db = await getDb(); return db ? db.select({ id: users.id, name: users.name, email: users.email, role: users.role, isActive: users.isActive }).from(users).orderBy(users.name) : []; }),
    userUpdate: permissionGuard(["admin", "sous_proved"], "audit.view").input(z.object({ userId: z.number(), name: z.string().optional(), email: z.string().email().optional(), isActive: z.boolean().optional() })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); const { userId, ...changes } = input; await db.update(users).set(changes).where(eq(users.id, userId)); await createAudit({ actorId: ctx.user.id, action: "USER_UPDATED", entityType: "user", entityId: userId, afterData: changes }); return { success: true }; }),
    userRoleUpdate: permissionGuard(["admin"], "audit.view").input(z.object({ userId: z.number(), role: z.enum(["user", "admin", "sous_proved", "secretariat", "chef_bureau", "ops", "inspecteur", "ecole"]) })).mutation(async ({ input, ctx }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" }); await db.update(users).set({ role: input.role }).where(eq(users.id, input.userId)); await createAudit({ actorId: ctx.user.id, action: "USER_ROLE_UPDATED", entityType: "user", entityId: input.userId, afterData: input }); return { success: true }; }),
  }),
  audit: router({
    entityHistory: permissionGuard(["admin", "sous_proved"], "audit.view").input(z.object({ entityType: z.string(), entityId: z.number() })).query(async ({ input }) => { const db = await getDb(); return db ? db.select().from(auditLogs).where(and(eq(auditLogs.entityType, input.entityType), eq(auditLogs.entityId, input.entityId))).orderBy(desc(auditLogs.createdAt)) : []; }),
